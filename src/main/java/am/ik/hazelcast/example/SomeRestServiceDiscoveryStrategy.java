...
public class SomeRestServiceDiscoveryStrategy extends AbstractDiscoveryStrategy {
	...
	public SomeRestServiceDiscoveryStrategy(DiscoveryNode discoveryNode, ILogger logger, //
		...
		this.baseUrl = getOrDefault("discovery.rest", DISCOVERY_URL,
				"http://localhost:12345/");
		this.applicationScope = getOrDefault("discovery.rest", APPLICATION_SCOPE,
				"hazelcast-cluster");

		logger.info("SomeRestService discovery strategy started {url=" //
				+ baseUrl + ", scope=" + applicationScope + "}");
		System.out.println("SomeRestService discovery strategy started {url=" //
				+ baseUrl + ", scope=" + applicationScope + "}");
	...
	public void start() {
		...
		System.out.println("Registering " + host + ":" + port);
